...
parser.add_argument('--dataset', type=str, default='oo3d',
                    help='Device to use. Like cuda, cuda:0 or cpu')
...
def compute_statistics_of_path_new(path, model, batch_size, dims, device,
                                   num_workers=1, reso=128, dataset='oo3d', save_path='results'):
    basepath=os.path.join(save_path, 'fid' + str(reso))
    ...
    try:
        try:
            ...
            files=[]
            path = pathlib.Path(os.path.join(path))

            for name in os.listdir(path):
                if os.path.isdir(os.path.join(path,name)):
                    img=os.path.join(path,name)
                    # random_files = [os.path.join(img, idd) for idd in os.listdir(img) if idd.endswith('.png')]
                    # rf = sorted(random.sample(random_files, 10))
                    # files = files+rf
                    # 随机选10个
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1, reso=128, dataset='oo3d', save_path='results'):
    ...
    m2, s2 = compute_statistics_of_path_new(paths[0], model, batch_size,
                                        dims, device, num_workers, reso=reso, dataset=dataset, save_path=save_path)
...
def main():
    ...
    calculate_fid_given_paths(args.path,
                                args.batch_size,
                                device,
                                args.dims,
