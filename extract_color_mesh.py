...
if __name__ == "__main__":
    ...
    kwargs = {'root_dir': f'{args.root_dir}',
              'img_wh': tuple(args.img_wh),
              'downsample':1.0,}
    if args.dataset_name == 'nerf':
    ...
    load_ckpt(model, f'{args.ckpt_path}')
    ...
    PlyData([PlyElement.describe(vertices_[:, 0], 'vertex'), 
             PlyElement.describe(face, 'face')]).write(f'results/{args.dataset_name}/{args.scene}.ply')

    # remove noise in the mesh by keeping only the biggest cluster
    print('Removing noise ...')
    mesh = o3d.io.read_triangle_mesh(f"results/{args.dataset_name}/{args.scene}.ply")
    ...
    if args.use_vertex_normal: ## use normal vector method as suggested by the author.
        ...
        for img_idx in tqdm(range(len(dataset))):
            ...
            # image = Image.open(dataset.image_paths[img_idx]).convert('RGB')
            # image = image.resize(tuple(args.img_wh), Image.LANCZOS)
            # image = np.array(image)

            ## read the camera to world relative pose
            P_c2w = np.concatenate([dataset.poses[img_idx], np.array([0, 0, 0, 1]).reshape(1, 4)], 0)
            ...
            rays_o = torch.FloatTensor(dataset.poses[img_idx][:, -1]).expand(N_vertices, 3)
            ...
            opacity = results['opacity'].cpu().numpy()[:, np.newaxis] # (N_vertices, 1)
    ...
    if args.use_vertex_normal:
        v_colors = results['rgb'].cpu().numpy() * 255
